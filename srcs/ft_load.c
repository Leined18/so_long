...
int	ft_spritesheet(t_info *data, char *path, sprite_type type)
{
	...
	printf("total_frames[%d]:		%d type: [%d]\n", type,
				data->spritesheetInfo.total_frames[type], type);
	printf("frames[%d]: 			%d type: [%d]\n", type,
					data->spritesheetInfo.frames[type], type);
	printf("rows[%d]:			%d type: [%d]\n", type,
					data->spritesheetInfo.rows[type], type);
	printf("width[%d]:			%d type: [%d]\n", type,
					data->spritesheetInfo.width[type], type);
	printf("height[%d]:			%d type: [%d]\n", type,
					data->spritesheetInfo.height[type], type);
	return (i);
}
