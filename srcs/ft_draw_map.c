...
static void	put_wall_img(t_info *data, int *j, int *i)
{
	mlx_put_image_to_window(data->mlx, data->win,
		data->images.sprites[WALL_SPRITE][0][0]->img, (*j) * RES, (*i) * RES);
}

static void	put_empty_img(t_info *data, int *j, int *i)
{
	mlx_put_image_to_window(data->mlx, data->win,
		data->images.sprites[FIELD_SPRITE][0][0]->img, (*j) * RES, (*i) * RES);
}
...
static void	put_player_img(t_info *data, int *j, int *i)
{
	if (data->direction == 'N')
		mlx_put_image_to_window(data->mlx, data->win,
			data->images.sprites[PLAYER_SPRITE][0][0]->img, (*j) * RES, (*i) * RES);
	else if (data->direction == 'S')
		mlx_put_image_to_window(data->mlx, data->win,
			data->images.sprites[PLAYER_SPRITE][DOWN][0]->img, (*j) * RES, (*i)
			* RES);
	else if (data->direction == 'E')
		mlx_put_image_to_window(data->mlx, data->win,
			data->images.sprites[PLAYER_SPRITE][0][0]->img, (*j) * RES, (*i)
			* RES);
	...
	while (data->map[++y])
	{
		...
		while (data->map[y][++x])
		{
			...
			if (tile == '1')
				...
				mlx_put_image_to_window(data->mlx, data->win,
					data->images.sprites[COLLECT_SPRITE][0][0]->img, x * RES, y
					* RES);
			else if (tile == 'E')
				mlx_put_image_to_window(data->mlx, data->win,
					data->images.sprites[EXIT_SPRITE][0][0]->img, x * RES, y * RES);
		}
	}
}
